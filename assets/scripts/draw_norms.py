"""
Normalization Methods — Visual Comparison  (transparent bg)
...
def box(ax, x, y, w, h, color, title, sub='', fs=9.5, fs2=7):
    b = FancyBboxPatch((x, y), w, h, boxstyle='round,pad=0.12',
                       facecolor=color, edgecolor='none', alpha=0.10, linewidth=0, zorder=1)
    ax.add_patch(b)
    b2 = FancyBboxPatch((x, y), w, h, boxstyle='round,pad=0.12',
                        facecolor='none', edgecolor=color, alpha=0.60, linewidth=1.1, zorder=2)
    ax.add_patch(b2)
...
for i, (name, color, desc, batch_info, use_case) in enumerate(methods):
    ...
    ax.text(0.5, y+0.38, desc, fontsize=7.5, color=C['dim'], family='monospace', zorder=3)
    ax.text(0.5, y+0.15, batch_info, fontsize=7.5, color=C['dim'], family='monospace', zorder=3)

    # Grid visualization
    if i == 0:   # BatchNorm: all spatial highlighted
        hl = {(r, c) for r in range(6) for c in range(10)}
    elif i == 1: # LayerNorm: one row of channels
        hl = {(0, c) for c in range(10)}
    elif i == 2: # GroupNorm: 4 group blocks
        hl = set()
        for g in range(4):
            for r in range(6):
                for c in range(g*2, min((g+1)*2, 8)):
                    hl.add((r, c))
    else:        # InstanceNorm: one spatial slice
        hl = {(r, c) for r in range(6) for c in range(10)}
    draw_grid(ax, 4.0, y+0.05, 2.8, 0.60, 6, 10, f'{name} — highlighted region', color, hl)

    # Use case badge
    ax.text(7.6, y+0.38, use_case, fontsize=8.5, color=color, family='monospace', fontweight='bold', zorder=3)
...
# ── Visual example (GroupNorm grouping) ──
ex_y = 1.3
...
for g in range(4):
    gx = 0.5 + g * 2.9
    ax.text(gx, ex_y+0.35, f'Group {g}', fontsize=7, color=C['orange'], family='monospace', ha='center', zorder=3)
    for r in range(3):
        for c in range(4):
            rect = FancyBboxPatch((gx+0.2 + c*0.32, ex_y - r*0.20), 0.28, 0.16,
                                  boxstyle='round,pad=0.02',
                                  facecolor=C['orange'], edgecolor='none', alpha=0.12+r*0.06, zorder=1)
            ax.add_patch(rect)
...
ax.text(6, ex_y-0.45, '→  2 channels per group, normalize together over H×W×2',
        fontsize=8, color=C['dim'], family='monospace', zorder=3)

plt.tight_layout(pad=0.3)
plt.savefig('../img/normalization.png', dpi=180, bbox_inches='tight', transparent=True, edgecolor='none')
plt.close()

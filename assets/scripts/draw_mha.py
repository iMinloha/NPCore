"""
MultiHeadAttention — Forward Pass Diagram  (transparent bg)
...
C = {'cyan': '#0d7377', 'blue': '#1a56db', 'purple': '#6d28d9',
     ...
     'gray': '#6b7280', 'fg': '#111827', 'dim': '#4b5563', 'white': '#ffffff'}
...
def box(ax, x, y, w, h, color, title, sub='', fs=9, fs2=6.5):
    ...
    b2 = FancyBboxPatch((x, y), w, h, boxstyle='round,pad=0.15',
                        facecolor='none', edgecolor=color, alpha=0.70, linewidth=1.2, zorder=2)
    ax.add_patch(b2)
...
for i, (col, lab) in enumerate(leg):
    ...
    ax.text(lx+0.20, 8.80, lab, fontsize=6.5, color=C['dim'], family='monospace', zorder=3)

plt.tight_layout(pad=0.3)
plt.savefig('../img/mha.png', dpi=180, bbox_inches='tight', transparent=True, edgecolor='none')
plt.close()

...
Matrix<float> RNN::forward(Matrix<float>& input) {
    seq_len = input.row;
    COREPP_ASSERT(input.col == input_size, "RNN input size mismatch");
...
Matrix<float> RNN::backward(Matrix<float>& grad_output) {
    COREPP_ASSERT(grad_output.row == seq_len, "RNN backward: seq_len mismatch");
    COREPP_ASSERT(grad_output.col == hidden_size, "RNN backward: hidden_size mismatch");

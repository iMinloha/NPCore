...
inline void RMSProp_method(std::vector<Module<float>*> params,Matrix<float>& grad,float lr,float beta=0.9f,float eps=1e-5f){
    ...
    for(int i=params.size()-1;i>=0;i--){auto*l=params[i];grad_=l->backward(grad_);
        ...
        for(size_t p=0;p<pl.size()&&p<gl.size();++p){auto*P=pl[p],*G=gl[p];if(!G)continue;
            if(p==0&&P->row==l->v.row&&P->col==l->v.col)P->fused_rmsprop_update(*G,l->v,beta,lr,eps);
            else *P=*P-(*G)*lr;}
        l->CleanGard();}}
}

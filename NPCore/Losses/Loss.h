...
template<typename T> T cross_entropy_loss(const Matrix<T>& lg, const Matrix<T>& t);
...
template<typename T> Matrix<T> bce_loss_grad(const Matrix<T>& p, const Matrix<T>& t);

template<typename T> Matrix<T> smooth_l1_loss_grad(const Matrix<T>& p, const Matrix<T>& t, T b = 1);

...
Matrix<T> Matrix<T>::operator+(Matrix<T> mat2) const {
    NPCORE_ASSERT(row == mat2.row && col == mat2.col && channel == mat2.channel, "Matrix shape mismatch");
...
template<typename T>
Matrix<T> Matrix<T>::operator/(Matrix<T> mat2) const {
    NPCORE_ASSERT(this->shape() == mat2.shape(), "Matrix shape mismatch");
    Matrix<T> res(row, col);
    int total = row * col;
#ifdef __AVX__
    int i = 0;
...
template<typename T>
Matrix<T> Matrix<T>::operator*(Matrix<T> mat2) const {
    NPCORE_ASSERT(col == mat2.row, "Matrix multiplication dimension mismatch");
    ...
    if constexpr (std::is_same_v<T, float>) {
        if (device_ == Device::GPU && mat2.device_ == Device::GPU) {
            ...
            result.data = (float*)NPCore::cuda_malloc_device(row * result.col * sizeof(float));
...
Matrix<T> &Matrix<T>::operator<<(T value) {
    if (currentIndex < row * col) {
        ...
        data[r * col + c] = value;
        currentIndex++;
    } else NPCORE_ASSERT(false, "Append out of range");
    return *this;
}
...
void Matrix<T>::Analysis(const std::string& title) {
    ...
    auto stat_val = [&](const char* label, double val) -> std::string {
        std::ostringstream oss;
        oss << label << "=";
        if (std::abs(val) < 1e-4) oss << std::scientific << std::setprecision(2) << val;
        else if (std::abs(val) < 0.01) oss << std::fixed << std::setprecision(6) << val;
        else if (std::abs(val) < 1) oss << std::fixed << std::setprecision(4) << val;
        else if (std::abs(val) < 100) oss << std::fixed << std::setprecision(3) << val;
        else oss << std::fixed << std::setprecision(1) << val;
        return oss.str();
    };

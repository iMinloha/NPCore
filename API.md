...
// 分类
Matrix<float> grad = cross_entropy_loss_grad(logits, labels);  // 带 softmax 的 CE 梯度
float ce = bce_loss(pred, target);             // 二分类交叉熵
...
| `cross_entropy_loss_grad` | `(N, classes)` | `(N, classes)` | 分类, logits 输入 |
...
```cpp
// 工厂函数 (使用默认超参数)
Optim sgd  = SGD(0.01f);        // lr=0.01
Optim adam = Adam(0.001f);      // lr=0.001, β₁=0.9, β₂=0.999
Optim rms  = RMSProp(0.01f);    // lr=0.01, β=0.99
...
```cpp
// 方式 1: 单样本反复训练
nn::Trainer trainer(*layer, nn::MSE, Adam(0.001f));
...
int main() {
    ...
    // 训练
    Optim optim(Adam_step, net.getParams(), 0.001f);
    Matrix<float> x, y;
    for (int e = 0; e < 200; e++) {
        loader.reset(); loader.train();
        float total = 0; int cnt = 0;
        while (loader.next_batch(x, y)) {
            auto out = net.forward(x);
            optim.step(cross_entropy_loss_grad(out, y));
            total += mse_loss(out, y); cnt++;
        }
